...
9. **Village Core** — A late-game block (crafted from emerald blocks, iron blocks, and a nether star) that builds a plains village from scratch, one jigsaw piece per Minecraft midnight, using vanilla's own village structure pool. Place it on or near the surface, supply RF from your cable network, and it lays a town center on the first midnight then expands outward from there — roads, houses, farms, decoration — indistinguishable from a naturally generated plains village. Each house earns one emerald per day; each villager and house costs RF to sustain. Fall short on energy and clustered villagers convert to pillagers. Track births, deaths, pillager conversions, population over time, and growth progress through the sneak-right-click stats GUI. Emeralds flow in and out via right-click (or hoppers/pipes) using a Storage Drawers-style interface. The Core is unbreakable once placed in survival — plan your location carefully. A **Creative Village Core** (creative-only, pink, not craftable) skips the RF requirement and adds a "Simulate Day" button for rapid testing.
