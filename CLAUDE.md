...
1. **Unbreakable stone & deepslate** *(Easy — build first)* — Cancel breaking of **only** `minecraft:stone` and `minecraft:deepslate` via `PlayerBlockBreakEvents.BEFORE` (return `false`). Everything else stays breakable — stone variants (granite, diorite, andesite, tuff), dirt, gravel, and all ores. Make them genuinely unbreakable.
2. **Large fleeing herds** *(Easy)* — Override biome spawn settings to raise pack sizes (sheep 15–25, cows 10–20, pigs 8–16, chickens 20–40, horses 6–12) and inject a `FleeEntityGoal` (detect players ~32 blocks, sprint away) into existing passive mobs. **Horses are exempt — they spawn in large herds but do not flee.** Inject goals via **mixins/events — do not replace vanilla entities** (preserves breeding, drops, pathfinding, animations).
