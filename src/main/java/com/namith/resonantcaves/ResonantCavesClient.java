	...
	public void onInitializeClient() {
		...
		ClientPlayNetworking.registerGlobalReceiver(OpenVillageCoreScreenPayload.ID, (payload, context) ->
				context.client().execute(() -> context.client().setScreen(new VillageCoreScreen(
						payload.pos(), payload.storedEnergy(), payload.houseCount(), payload.villagerCount(), payload.sustained()))));

		ClientPlayNetworking.registerGlobalReceiver(VillageCoreStateUpdatePayload.ID, (payload, context) ->
				context.client().execute(() -> {
					if (context.client().currentScreen instanceof VillageCoreScreen screen && screen.getPos().equals(payload.pos())) {
						screen.updateState(payload.storedEnergy(), payload.houseCount(), payload.villagerCount(), payload.sustained());
					}
				}));
	}
